...
Get-PackageParameters -Parameters "/Release:'alpha','beta','rc'" | Set-Variable pp
if (!$pp['Release'] -or [String]::IsNullOrWhiteSpace($pp['Release'])) { $pp['Release'] = "alpha","beta","rc" }
$disallowedReleaseTypes = (Compare-Object "alpha","beta","rc" $pp['Release'] | Select-Object -ExpandProperty InputObject)

$getDownloadUrl = {
  WriteOutput("Checking maven repository for latest aapt2 version.")
  $releaseTypes = If ($args[0].Count -gt 1) {$args[0]} Else { $args }
  [Regex] $regex = '(?i)(' + (($releaseTypes | ForEach-Object {[regex]::escape($_)}) -join "|") + ')'
  $mavenAapt2Url = 'https://dl.google.com/dl/android/maven2/com/android/tools/build/aapt2'
  $aapt2Url = "$mavenAapt2Url/maven-metadata.xml"
  [xml]$data = (New-Object System.Net.WebClient).DownloadString($aapt2Url)
  $versions = $data.metadata.versioning.versions.version
  $matchedVersions = $versions -notmatch $regex
  $latest = $matchedVersions | Sort-Object -Descending | Select-Object -First 1
  return "$mavenAapt2Url/$latest/aapt2-$latest-windows.jar"
}
...
$url        = & $getDownloadUrl $disallowedReleaseTypes # download url, HTTPS preferred
...
$packageArgs = @{
  ...
  # softwareName  = 'aapt*' #part or all of the Display Name as you see it in Programs and Features. It should be enough to be unique

  # Checksums are now required as of 0.10.0.
  # To determine checksums, you can get that from the original site if provided. 
  # You can also use checksum.exe (choco install checksum) and use it 
  # e.g. checksum -t sha256 -f path\to\file
  # checksum      = '81814A9CB0766AD59FDD9C3D150578223C79D6172D0B130CC2A67CA61B301BA9'
